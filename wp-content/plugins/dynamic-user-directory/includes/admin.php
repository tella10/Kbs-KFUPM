...
<?php if ( in_array( 'dynamic-user-directory-export/dynamic-user-directory-export.php' , $dud_plugin_list ) ) 
{ ?>
...
<div class="dud-settings-section">
	<table class="form-table">
		...
		<tr>
			...
			<td>This link is shown when the directory initially loads. Clicking this will trigger the creation of the full directory export file. The page will then refresh and the directory download link will appear.</td>
			<td></td>
		</tr>
		<tr>
				 ...
				 <tr id="meta_fld_<?php echo $inc; ?>">
					...
					<td><b>Format Meta Field <?php echo $inc; ?> As</b><br>
						<select name="<?php echo $dud_option_name;?>[dud_fld_format_<?php echo $inc; ?>]" id="dud_hyperlink_flds">
							...
							<OPTION value="35" <?php echo (!empty($dud_options['dud_fld_format_' . $inc]) && $dud_options['dud_fld_format_' . $inc] == "35") ? "SELECTED" : ""; ?>>Multiple Values => Bulleted (Hide MP hyphens)</OPTION>
							<OPTION value="36" <?php echo (!empty($dud_options['dud_fld_format_' . $inc]) && $dud_options['dud_fld_format_' . $inc] == "36") ? "SELECTED" : ""; ?>>Multiple Values => Comma Delimited (Hide MP hyphens)</OPTION>		
...
function my_plugin_notice() {
    ...
    if ( !get_user_meta( $user_id, 'dud_export_addon_notice_dismissed' ) && current_user_can('administrator') )
	{		
		$current_url = esc_url( home_url( '/' ) ) . 'wp-admin/options-general.php?page=user_directory&';
		echo '<div class="notice notice-warning"><p>Dynamic User Directory has a new <a href="http://sgcustomwebsolutions.com/wordpress-plugin-development/" target="_blank">Directory Export Add-on</a> available now!&nbsp;&nbsp;<a href="' . $current_url . 'dud-plugin-dismissed">Dismiss</a></p></div>';
	}
}
...
function my_plugin_notice_dismissed() {
    ...
    if ( isset( $_GET['dud-plugin-dismissed'] ) )
        add_user_meta( $user_id, 'dud_export_addon_notice_dismissed', 'true', true );
}
