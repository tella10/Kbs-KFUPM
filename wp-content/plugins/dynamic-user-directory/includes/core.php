	...
	{   		
	   ...
	   {	
	    ...
	    {          	
			...
			if($user_directory_border === "surrounding_border")
				$user_contact_info .= "<DIV style='font-size:" . $user_directory_listing_fs . "px;padding-left: " . $user_directory_avatar_padding . "px;letter-spacing: " . $ud_letter_spacing . ";' class='dir-listing-text-surr-border'>\n\t\t";
			else
			    $user_contact_info .= "<DIV style='font-size:" . $user_directory_listing_fs . "px;padding-left: " . $user_directory_avatar_padding . "px;letter-spacing: " . $ud_letter_spacing . ";' class='dir-listing-text'>\n\t\t";
		}
		else	
			$user_contact_info .= "\n\t<DIV style='font-size:" 
				. $user_directory_listing_fs . "px;letter-spacing: " . $ud_letter_spacing . ";' class='dir-listing-text-no-avatar'>\n\t\t";
...
function dynamic_ud_format_meta_val($user_meta_fld, $dud_options, $format)
{
	...
	* 34    = Image
	* 35    = Multiple Value List => Bulleted (Hide MP Hyphens)
	* 36    = Multiple Value List => Comma Delimited (Hide MP Hyphens)
	**********************************************/
	...
	else if(!is_array($user_meta_fld) && ($format === "4" || $format === "7" || $format === "8" || $format === "9"))
	{
		if(strlen($user_meta_fld) > 2 && substr($user_meta_fld, 0, 2) === "a:")
			return implode(", ",(unserialize(stripslashes($user_meta_fld))));
										  ...
										  || $format === "12" || $format === "13" || $format === "14" || $format === "15" || $format === "35" ))
	{
		...
		if(strlen($user_meta_fld) > 2 && substr($user_meta_fld, 0, 2) === "a:")
		{
			...
			{					
				foreach($list_items  as $listitem)
				{
					...
					if($format === "5" || $format === "35" || $format === "10" || $format === "11" || $format === "12"  )
			...
			{			
				...
				else                 //add key-value pair to the meta fld var
				{
					...
					/*if(sizeof($user_meta_fld) == 1)
						$parsed_val .= $value;
					else*/ if($format === "5" || $format === "35") //bulleted list
					{
						if($ud_display_listings !== "horizontally" || $inc > 1)
							$parsed_val .= "<BR>";	
					...
					else if($format === "4" || $format === "36") //comma delimited list
					{
						if($inc > 1) $parsed_val .= ", ";
			...
			for($met=0; $met < sizeof($user_meta_fld); $met++) 
			{
				if($user_meta_fld[$met])
				{
					...
					else                               //add the item to the meta fld var
					{
						if($format === "35" || $format === "36")
						{
							$user_meta_fld[$met] = str_replace ( "-", " ", $user_meta_fld[$met]);
							$user_meta_fld[$met] = ucwords($user_meta_fld[$met]);
						}
					
						if(sizeof($user_meta_fld) == 1)
							$parsed_val .= $user_meta_fld[$met];
						else if($format === "5" || $format === "35" || $format === "10" || $format === "11" || $format === "12" ) //bulleted list
						{
							if($ud_display_listings !== "horizontally" || $inc > 1)
								$parsed_val .= "<BR>";	
						...
						else if($format === "4" || $format === "36" || $format === "7" || $format === "8" || $format === "9" ) //comma delimited list
						{
							if($inc > 1) $parsed_val .= ", ";
