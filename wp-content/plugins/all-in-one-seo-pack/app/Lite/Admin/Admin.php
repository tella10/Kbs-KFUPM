	...
	protected function addAdminBarMenuItems() {
		...
		if ( current_user_can( 'update_plugins' ) ) {
			$this->adminBarMenuItems['aioseo-pro-upgrade'] = [
				...
				'href'   => apply_filters( 'aioseo_upgrade_link', aioseo()->helpers->utmUrl( AIOSEO_MARKETING_URL . 'lite-upgrade/', 'admin-bar' ) ),
				'meta'   => [ 'target' => '_blank' ],
			];
		}
	...
	public function addMenu() {
		...
		if ( current_user_can( 'aioseo_manage_seo' ) ) {
			...
			$submenu[ $this->pageSlug ][] = [
				...
				apply_filters( 'aioseo_upgrade_link', aioseo()->helpers->utmUrl( AIOSEO_MARKETING_URL . 'lite-upgrade/', 'admin-menu' ) )
			];
		}
	}
}