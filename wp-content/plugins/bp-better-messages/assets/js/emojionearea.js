...
; ( function ( factory, global ) {
    ...
    function getDefaultOptions () {
        if ($.fn.BPemojioneArea && $.fn.BPemojioneArea.defaults) {
            return $.fn.BPemojioneArea.defaults;
        }
    ...
    function updateRecent(self, show) {
        ...
        if (!self.recent || self.recent !== emojis || show) {
            if (emojis.length) {
                ...
                $(items).insertAfter(self.recentCategory.children(".bp-emojionearea-category-title"));
...
// see https://github.com/Modernizr/Modernizr/blob/master/feature-detects/storage/localstorage.js
    ...
    function init(self, source, options) {
        ...
        $.each(options.filters, function(filter, params) {
            ...
            do {
                ...
                category.html(items);
                $('<div class="bp-emojionearea-category-title"/>').text(params.title).prependTo(category);
            } while (--skin > 0);
        });
        ...
        attach(self, picker.find(".bp-emojionearea-filter"), {click: "filter.click"});
        ...
        self.on("@filter.click", function(filter) {
            ...
            .on("@change", function() {
                var html = self.editor.html().replace(/<\/?(?:div|span)[^>]*>/ig, '');
                // clear input: chrome adds <br> when contenteditable is empty
                if (!html.length || /^<br[^>]*>$/i.test(html)) {
                    self.editor.html(self.content = '');
                }

                source[sourceValFunc](html);
            })
            ...
            .on("@blur", function() {
                ...
                if($(event.relatedTarget).closest('.bpbm-medium-editor-toolbar').length > 0){
                    //skip
        ...
        if (options.search) {
            self.on("@search.focus", function() {
                ...
                .on("@search.keypress", function(hide) {
                    var filterBtns = picker.find(".bp-emojionearea-filter");
    ...
    $.fn.BPemojioneArea = function(options) {
        return this.each(function() {
            if (!!this.BPemojioneArea) return this.BPemojioneArea;
            $.data(this, 'BPemojioneArea', this.BPemojioneArea = new EmojioneArea($(this), options));
            return this.BPemojioneArea;
        });
    };

    $.fn.BPemojioneArea.defaults = getDefaultOptions();

    $.fn.BPBMloadEmojione = function(options){
        loadEmojione(options);
    };

    $.fn.BPemojioneAreaText = function(options) {
        options = getOptions(options);
